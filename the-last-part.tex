...
$aucun$ will thus end up being $N \limp ((NP\_NEG \limp S) \limp S)$
instead of the usual $N \limp ((NP \limp S) \limp S)$.\footnote{Strictly
  following the EPTD for $aucun$ would lead us to give it a type
  $DET\_NEG$ and then give the two types $DET \limp ((NP \limp S) \limp
  S)$ and $DET\_NEG \limp ((NP\_NEG \limp S) \limp S)$ to nouns. In this
  demonstration, we prefer to align our examples with the treatment
  which is customary to ACG research.}  The type of $ne$ will be more
verbose, as it will consume a verb and transform its valency so that it
demands that exactly one of its $NP$ arguments contains a negative
determiner.

$$
N_{ne_{tv_1}} : (NP \limp NP \limp S) \limp (NP\_NEG \limp NP \limp S)
$$
$$
N_{ne_{tv_2}} : (NP \limp NP \limp S) \limp (NP \limp NP\_NEG \limp S)
$$
...
$$
N_{ne_{tv_3}} : (NP \limp NP \limp S) \limp (NP\_NEG \limp NP\_NEG \limp S)
$$
...
$$
N_{que_1} : (NP \limp S) \limp N \limp N
$$
$$
N_{que_2} : (NP \limp S) \limp N\_NEG \limp N\_NEG
$$
$$
N_{de_1} : NP \limp N \limp N
$$
$$
N_{de_2} : NP \limp N\_NEG \limp N\_NEG
$$
$$
N_{de_3} : NP\_NEG \limp N \limp N\_NEG
$$
...
$NP$, a noun phrase with no negative determiner. This means that
$\lambda^{\circ} x.\ N_{chasse}\ x\ y$ has type $NP \limp S$ which makes
it an invalid argument for the function $N_{aucun}\ N_{loup}$, which has
type $(NP\_NEG \limp S) \limp S$. Even supposing that we would be able
to type the relative clause with $NP \limp S$, the complementizer $que$
would map $N_{le}\ N_{tatou}$ from type $NP$ to type $NP$, preserving
the property that the noun phrase does not contain a negative
determiner. This would then make it incompatible with
$N_{ne_{iv}}\ N_{court}$ which expects its argument to be an
$NP\_NEG$. By misplacing the paired grammatical words, both the main
clause and the relative clause become untypable.
...
\begin{exe}
  ...
  \ex \label{ex:good-neg-embed} Jean n'aime l'odeur d'aucun tatou. \\
      $(N_{le}\ (N_{de_3}\ (N_{aucun}\ N_{tatou})\ N_{odeur}))\ (\lambda^{\circ} y.\ N_{ne_{tv_2}}\ N_{aime}\ N_{Jean}\ y)$
  \ex \label{ex:good-neg-rel} Le tatou qu'aucun loup ne chasse court. \\
      $(N_{le}\ (N_{que}\ (\lambda^{\circ} y.\ (N_{aucun}\ N_{loup})\ (\lambda^{\circ} x.\ N_{ne_{tv_1}}\ N_{chasse}\ x\ y))\ N_{tatou}))\ (\lambda^{\circ} x.\ N_{court}\ x)$
  \ex * \label{ex:bad-neg-noneg} Jean n'aime le tatou. \\
      ? $(N_{le}\ N_{tatou})\ (\lambda^{\circ} y.\ N_{ne_{tv_?}}\ N_{aime}\ N_{Jean}\ y)$
  \ex * \label{ex:bad-neg-rel} Le tatou qu'aucun loup chasse ne court. \\
      ? $(N_{le}\ (N_{que}\ (\lambda^{\circ} y.\ (N_{aucun}\ N_{loup})\ (\lambda^{\circ} x.\ N_{chasse_?}\ x\ y))\ N_{tatou}))\ (\lambda^{\circ} x.\ N_{ne_{iv_?}}\ N_{court}\ x)$
\end{exe}
...
$NP\_VAR$, which stands for ``empty'' noun phrases introduced by
extraction, and $NP$, which stands for the other noun phrases, i.e.
proper names and nouns with determiners. The grammar gives no constant
capable of constructing a value of type $NP\_VAR$. The only way to
obtain a term with this type is to abstract over a variable having the
type. Then, the types of relative pronouns end up being $(NP\_VAR \limp
S) \limp N \limp N$, ensuring that the relative clause given as the
first argument is a $\lambda$-abstraction whose argument has type
$NP\_VAR$.
...
$$
$$
E_{dort_2} : NP\_VAR \limp S\_ROOT
$$
$$
E_{aime_1} : NP \limp NP \limp S
$$
$$
E_{aime_2} : NP\_VAR \limp NP \limp S\_ROOT
$$
$$
E_{aime_3} : NP \limp NP\_VAR \limp S\_ROOT
$$
$$
E_{dit\ que_1} : NP \limp S \limp S
$$
$$
E_{dit\ que_2} : NP\_VAR \limp S \limp S\_ROOT
$$
$$
E_{dit\ que_3} : NP \limp S\_ROOT \limp S\_EMB
$$
$$
E_{dit\ que_4} : NP \limp S\_EMB \limp S\_EMB
$$
...
$$
E_{qui} : (NP\_VAR \limp S\_ROOT) \limp N \limp N
$$
$$
E_{que_1} : (NP\_VAR \limp S\_ROOT) \limp N \limp N
$$
$$
E_{que_2} : (NP\_VAR \limp S\_EMB) \limp N \limp N
$$

The above types ensure that the abstracted variables corresponding to
traces of extracted constituents will be given the type $NP\_VAR$, that
$qui$ can only be used to extract constituents from root positions while
$que$ allows for extraction from both root and embedded
...
Given this type system, we can assign the type $S$ to the term
...
used to extract a subject from a clause embedded in the relative clause.
The problem with sentence (\ref{ex:bad-ext}) is that the type of the
relative clause is $NP\_VAR \limp S\_EMB$ which is not a valid argument
for any constant representing the relative pronoun $qui$. Typing the
variable $t$ with $NP$ instead of $NP\_VAR$ and using $E_{aime_1}$
instead of $E_{aime_2}$ would not help, since the resulting relative
clause would then have type $NP \limp S$, which would not admitted by
any relative pronoun.

\begin{exe}
  \ex * \label{ex:bad-ext} Le tatou qui Jean dit que \_ aime Marie dort. \\
      ? $(E_{le}\ (E_{qui_?}\ (\lambda^{\circ} t.\ E_{dit\ que_3}\ E_{Jean}\ (E_{aime_2}\ t\ E_{Marie}))\ E_{tatou}))\ (\lambda^{\circ} x.\ E_{dort_1}\ x)$
  \ex \label{ex:good-ext} Le tatou que Jean dit que Marie aime \_ dort. \\
      $(E_{le}\ (E_{que_2}\ (\lambda^{\circ} t.\ E_{dit\ que_3}\ E_{Jean}\ (E_{aime_3}\ E_{Marie}\ t))\ E_{tatou}))\ (\lambda^{\circ} x.\ E_{dort_1}\ x)$
\end{exe}
...
$$
$$
A_{le} : N\_SG \limp ((NP\_SG \limp S) \limp S)
$$
$$
A_{les} : N\_PL \limp ((NP\_PL \limp S) \limp S)
$$
$$
A_{dort} : NP\_SG \limp S
$$
$$
A_{dorment} : NP\_PL \limp S
$$
$$
A_{aime_1} : NP\_SG \limp NP\_SG \limp S
$$
$$
A_{aime_2} : NP\_SG \limp NP\_PL \limp S
$$
$$
A_{aiment_1} : NP\_PL \limp NP\_SG \limp S
$$
$$
A_{aiment_2} : NP\_PL \limp NP\_PL \limp S
$$
$$
A_{qui_1} : (NP\_SG \limp S) \limp N\_SG \limp N\_SG
$$
$$
A_{qui_2} : (NP\_PL \limp S) \limp N\_PL \limp N\_PL
$$
$$
A_{de_1} : NP\_SG \limp N\_SG \limp N\_SG
$$
$$
A_{de_2} : NP\_PL \limp N\_SG \limp N\_SG
$$
$$
A_{de_3} : NP\_SG \limp N\_PL \limp N\_PL
$$
$$
A_{de_4} : NP\_PL \limp N\_Pl \limp N\_PL
$$
...
has to be $NP\_SG$ since it is used as an argument to $A_{dort}$. This
means that the relative clause $dort$ has type $NP\_SG \limp
S$. However, there is no type for $qui$ which would allow us to modify
the plural noun $A_{tatous}$ of type $N\_PL$ by a relative clause with a
missing singular noun phrase, which has the type $NP\_SG \limp S$.
