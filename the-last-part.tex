...
sure that there is no interaction. In the rest of this chapter, we will
...
We will turn to another solution to the problem of complex grammars that
try to implement multiple independent constraints. This solution will be
based on a generalization of ACGs, but in order to introduce this
generalization, we will first need to present the manner in which ACGs
and their compositions are formulated in ACG literature.
...
the theme of this chapter.\footnote{Incidentally, this was not the
  reason for the way we presented ACGs in~\ref{sec:acg}. We chose this
  style as it grouped together the information in a more natural way. We
  approach signatures as descriptions of domains (strings, syntax,
  semantics), each one being associated with a distinguished type
  telling us which objects the domain has set out to describe (strings,
  sentences, truth values). Having distinguished types directly attached
  to signatures allows us to omit them from the ACG quadruples,
...
mapping their parent's language by the lexicon linking the two. By
homomorphism of the lexicons, we can find distinguished types for all
the non-root nodes. They are simply the images of their parents'
distinguished types by the connecting lexicons. For all of the nodes, it
is true that every term in the language assigned to the node has the
node's distinguished type.
...
property that is also true in the non-root cases due to the homomorphic
properties of lexicons. If we wanted to unify the definitions of the
root language and the non-root languages, we would have to check that a
term of the language has the distinguished type and that it has an
antecedent in the parent, if it has any parent. A relational statement
of this unification could look like this.
...
\section{Graphical Abstract Categorial Grammars}

In this section, we distill the reasoning about generalizing ACG
diagrams to graphs in a definition of an extension for ACGs. We then
examine its key properties and implications.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Let $L_1$ and $L_2$ be two extrinsic languages defined by the
      ...
      $\mathcal{E}_{\mathcal{G}'}(u_1) = \mathcal{E}_{\mathcal{G}}(u_1)$
      and $\mathcal{E}_{\mathcal{G}'}(u_2) =
      \mathcal{E}_{\mathcal{G}}(u_2)$ and finally that
  ...
  \item Let $L \subseteq \Lambda(\Sigma_O)$ be a language which is the
    ...
    $\mathcal{L}$. Now we verify that $\mathcal{E}_{\mathcal{G}'}(v) =
    L$.
...
this chapter. The lexicons for the edges connecting these constraint
nodes to $Synt$ all behave according to the scheme below:
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{0.4\textwidth}
    ...
    \caption{{\label{fig:puzzle-solution} The graph of a grammar
        enforcing all of the constraints discussed in this chapter.}}
  \end{subfigure}
  \qquad
  \begin{subfigure}[b]{0.4\textwidth}
    \centering
...
\subsection{On Alternative Interpretations of Graphical ACGs}
...
\begin{description}
  ...
  \item[Is the set of pangraphical languages included in the set of
    ...
    That is an interesting question indeed, whose resolution is left as
    an exercise for the reader.

  \item[Pangraphical languages are not ``less decidable'' than object
    languages.] \hfill \\

    Presuming that type checking is decidable for all the signatures of
    the graphical ACGs and that we can parse effectively for all the ACG
    edges in the graph, we can reuse the parsing algorithm we introduced
    in \ref{ssec:graphical-decidability}. We apply this algorithm to
    enumerate all the possible extrinsic parses in a language
    $\mathcal{E}_{\mathcal{G}}(v)$ and keep only those where for each
    ancestor $u$ we have the same abstract term for all paths from $u$
    to $v$.
\end{description}


\subsection{On the Practical Consequences of Graphical ACGs}

In this final subsection, we will enumerate some of the practical
benefits and challenges of using graphical ACGs for implementing
